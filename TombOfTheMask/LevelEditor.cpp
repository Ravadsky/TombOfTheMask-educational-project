...
void LevelEditor::Update()
{
	...
	int xMousePos = (event.mouseButton.x + (int)CameraOffset.x - (int)CAMERA_PIVOT.x + SPRITE_GAME_SIZE / 2) / SPRITE_GAME_SIZE;
	int yMousePos = (event.mouseButton.y + (int)CameraOffset.y - (int)CAMERA_PIVOT.y + SPRITE_GAME_SIZE / 2) / SPRITE_GAME_SIZE;

	//Ограничение позиций мыши по игровому полю
	if (xMousePos < 0 or xMousePos >= MAX_LEVEL_SIZE) xMousePos = -1;
	if (yMousePos < 0 or yMousePos >= MAX_LEVEL_SIZE) yMousePos = -1;
	...
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::Up)) CameraOffset += {0.f, -8.f};
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::Down)) CameraOffset += {0.f, 8.f};
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::Left)) CameraOffset += {-8.f, 0.f};
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::Right)) CameraOffset += {8.f, 0.f};
	...
	if (sf::Event::MouseButtonPressed and event.mouseButton.button == sf::Mouse::Left)
	{
		if (xMousePos != -1 and yMousePos != -1 and ActorsInfo[xMousePos][yMousePos].ActorID == -1)
		{
			...
			Actors[xMousePos][yMousePos] = GLevelSubsystem->CreateObject({ (float)xMousePos, (float)yMousePos }, (float)CurrentRotationAngle, CurrentObjectIndex);
		}
	}
	...
	if (sf::Event::MouseButtonPressed and event.mouseButton.button == sf::Mouse::Right)
	{
		if (xMousePos != -1 and yMousePos != -1)
		{
			...
			if (auto actor = Actors[xMousePos][yMousePos].lock()) actor->MarkToKill();
		}
	}
